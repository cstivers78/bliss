...
Bliss = (function() {
  var clone, defaults, tokenizer, escapeHtml;
  ...
  Bliss.prototype.compile = function(source, options) {
    ...
    context.raw = function(value) {
      if (value == null) return '';
      value.__bliss_raw = true;
      return value;
    };
    context.escape = escapeHtml;
    writer = new Writer();
    writer.write(tokenizer.tokenize(source));
    tmplParams = writer.parameters;
    
    // Add automatic escaping wrapper around expressions
    if (options.escapeByDefault) {
      writer.wrapExpressions(function(expr) {
        return '(function(v) { return v && v.__bliss_raw ? v : escape(v); })(' + expr + ')';
      });
    }
