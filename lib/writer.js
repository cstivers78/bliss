...
Writer = (function() {
  ...
  function Writer(options) {
    ...
    this.escapeByDefault = this.options.escapeByDefault !== false; // escape by default unless explicitly disabled
  }
    ...
    switch (tag.name) {
      ...
      case 'Value':
        ...
        if (tag.raw) {
          ...
          this.code('if(this.escapeByDefault){');
          this.code('Array.isArray(__tmp) ? write(escapeHTML(__tmp.join(""))) : write(escapeHTML(__tmp));');
          this.code('} else {');
          this.code('Array.isArray(__tmp) ? write(__tmp.join("")) : write(__tmp);');
          this.code('}');
        }
  ...
  Writer.prototype.source = function(context) {
    ...
    return [
      ...
      ctx.join(''),
      ';',
      'function escapeHTML(str){if(typeof str!=="string")str=String(str);return str.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/\'/g,"&#39;");}',
      this.buffer.join(''),
      'return __out.join(\'\');'
    ].join('');
  };
